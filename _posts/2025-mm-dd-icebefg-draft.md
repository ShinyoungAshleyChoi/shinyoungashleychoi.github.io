...
- 데이터셋의 파일들을 통일된 테이블로 표현하기 위해 구조화하는 방법
- 테이블 포맷이란 RDBMS의 이 등장한 이후로 존재해왔지만 예전부터 "테이블 포맷"이라는 용어를 사용한 것은 아님
...
- SQL이나 애드혹 스트립트를 던질 때 어떤 데이터가 어떤 테이블에 포함되어 있는 지를 매번 일일이 확인하고 싶지는 않을 것
- 그래서 "이 테이블에는 어떤 데이터가 있는지"를 이해하는 표준 방법이 필요했음
- 단, 이 
...
#### 저수준 테이블 운영 (Row-level table operations)
#### 타임 트래블
#### 버전 롤백
